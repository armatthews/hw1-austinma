...
public class AnnotationPrinter extends CasConsumer_ImplBase implements CasObjectProcessor {
  ...
  /**
   * Finds the number of spaces in sentence up to (but not including) the posth character. This is
   * needed to convert from raw string indices to the expected output format
   * 
   * @param sentence
   *          The sentence being analyzed
   * @param pos
   *          The position in the sentence up to which to count spaces
   * @return The number of spaces before pos in the sentence
   */
  protected static int getOffset(String sentence, int pos) {
    int offset = 0;
    for (int i = 0; i < pos; i++)
  ...
  public synchronized void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    Iterator<Annotation> annotationIter = jcas.getAnnotationIndex().iterator();
    while (annotationIter.hasNext()) {
      Annotation annot = (Annotation) annotationIter.next();
      if (annot instanceof GeneAnnotation) {
        GeneAnnotation geneAnnot = (GeneAnnotation)annot;
        // get the text that is enclosed within the annotation in the CAS
        String aText = String.format("%s|%d %d|%s", geneAnnot.getSentenceID(), geneAnnot.getBegin(), geneAnnot.getEnd(), geneAnnot.getTextSpan());
        try {
